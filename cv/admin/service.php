...
if ($edit != '') {
    $results = $db->query("SELECT * FROM t_service where id={$edit}");
...
// INSERT
if ($hTitle != '' && $hId == '') {
    $db->query("INSERT INTO t_service (title, desc) VALUES ('{$hTitle}', '{$hDescription}')");
}

// UPDATE
if ($hTitle != '' && $hId != '') {
    $db->query("UPDATE t_service SET title = '{$hTitle}', desc = '{$hDescription}' WHERE id ={$hId}");
    header("location:service.php");
}

// DELETE
if($delete != ''){
    $db->query("DELETE FROM t_service WHERE id = {$delete}");
    header("location:service.php");
}
                                    ...
                                    <table id="zero-config" class="table style-1 table-hover non-hover" style="width:100%">
                                        ...
                                        <tbody>
                                            <?php
                                                ...
                                                $results = $db->query('SELECT * FROM t_service');
                                                while ($row = $results->fetchArray()) {
                                                    print("<tr>");
                                                    ...
                                                    <td class="text-center">
                                                        <div class="dropdown custom-dropdown">
                                                            ...
                                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuLink2">
                                                                <a class="dropdown-item" href="service.php?edit='.$row['id'].'">Edit</a>
                                                                <a class="dropdown-item" href="service.php?delete='.$row['id'].'">Delete</a>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    ');
